...
This script is the OLD training script; originally I was coding in the approach of training would occur via a Lambda
...
# Fractions for time-based split
TRAIN_FRAC = float(os.environ.get("TRAIN_FRAC", "0.7"))
VAL_FRAC = float(os.environ.get("VAL_FRAC", "0.15"))  # test gets the rest


def load_features_from_s3(bucket: str, key: str) -> pd.DataFrame:
    log.info(f"Loading features from s3://{bucket}/{key}")
    try:
...
def prepare_data(df: pd.DataFrame):
    ...
    - Select feature columns (drop 20-day ones for now)
    - One-hot encode symbol
    """
    # Drop rows with no target
    df = df.dropna(subset=["target_log_return"]).copy()

    # Feature candidates (skip ret_mean_20, ret_std_20 for now)
    base_features = [
        "adj_close",
        "volume",
        "log_ret_1d",
    ...
    log.info(f"Using {len(feature_cols)} numeric features + {sym_dummies.shape[1]} symbol dummies")
    return X, y, meta, list(X.columns)


def time_based_split(X: pd.DataFrame, y: np.ndarray, meta: pd.DataFrame):
    """
    Split train/val/test based on unique dates across all symbols.
    """
    ...
    log.info(f"Split info: {split_info}")
...
def eval_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> dict:
    if len(y_true) == 0:
        return {"rmse": None, "mae": None, "smape": None, "directional_accuracy": None, "n_samples": 0}
...
def save_model_to_s3(model, bucket: str, prefix: str, run_id: str) -> str:
    out_key = f"{prefix}/xgboost/{run_id}/model.joblib"
    buf = io.BytesIO()
    dump(model, buf)
    buf.seek(0)
    S3.put_object(
...
def save_metrics_to_s3(metrics: dict, bucket: str, prefix: str, run_id: str) -> str:
    out_key = f"{prefix}/xgboost/{run_id}/metrics.json"
    S3.put_object(
        Bucket=bucket,
        Key=out_key,
    ...
    log.info(f"Saved metrics to s3://{bucket}/{out_key}")
    return out_key
...
def main():
    ...
    log.info(f"Loaded {len(df)} rows of features in {t_load:.3f}s")

    # 2) Prepare features/targets
    X, y, meta, feature_names = prepare_data(df)

    # 3) Time-based split
    (X_tr, y_tr, meta_tr), (X_val, y_val, meta_val), (X_te, y_te, meta_te), split_info = time_based_split(X, y, meta)

    # 4) Baseline: predict zero
    baseline_metrics = {}
    if len(y_val) > 0:
    ...
    log.info(f"Baseline (zero) test metrics: {baseline_test}")

    t_train_start = time.perf_counter()
    model = XGBRegressor(
        n_estimators=int(os.environ.get("XGB_N_ESTIMATORS", "300")),
        ...
        eval_metric="rmse",
        early_stopping_rounds=20 if len(y_val) > 0 else None,  # Moved here
    )
    ...
    log.info(f"Trained XGBoost in {t_train:.3f}s")
    ...
    log.info(f"XGBoost test metrics: {metrics_xgb['test']}")
